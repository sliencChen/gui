...
void uiEditItem::OnBuildWindowStructureCode()
{
	...
	if (!strcmp(Parent_win->stb_window->class_name, EDIT_CLASS_NAME))
...
void uiEditItem::OnBuildCreatingCode()
{
	...
	AddBuildingCode(wxString::Format(wxT("UIEditData *%s_data = %s->private_data;\n"), GetStrPropValue(PROP_NAME), m_Parent->GetStrPropValue(PROP_VAR_NAME)));
	AddBuildingCode(wxString::Format(wxT("sizer_add_window(((UIEditData *)%s->private_data)->list_sizer, %s, %s->rect.size, 1, 1, UI_ALIGN_CENTER_VER|UI_SIZER_FLAG_BORDER_TOP|UI_SIZER_FLAG_EXPAND);\n"), \
